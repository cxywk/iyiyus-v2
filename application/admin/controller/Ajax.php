...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        if ($chunkid) {
            ...
            if(strpos($url,".ipa")){
                $res=Category::ipaInfo($_SERVER['DOCUMENT_ROOT'].$url);
                $res = json_decode($res->getContent(),true);
                $fullurl = cdnurl($attachment->url, true);

                $attachment->url = str_replace($_SERVER['DOCUMENT_ROOT'], '', $res['url']);
                $attachment->save();
                $this->success(__('Uploaded successful'), '', [
                    'url'      => $res['url'],
                    'name'     => $res['name'],
                    'baoming'  => $res['sys_name'],
                    'banben'   => $res['version'],
                    'logo'     => $res['icon'], 
                    'filesize' => $show_filesize,
                    'fullurl'  => $fullurl
                ]);
                ...
                $this->success(__('Uploaded successful'), '', ['url' => $url, 'fullurl' => cdnurl($attachment->url, true)]);
            }
